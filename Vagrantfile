...
###########################################################################
# ddd-laravel-doctrineという記述の部分はprojectに合わせて適宜命名ください #
###########################################################################
...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder ".", "/home/ddd-laravel-doctrine", create:true, owner: "vagrant", group:"wheel", mount_options: ['dmode=777','fmode=777']
  
  # VitrualBoxの設定、仮想マシンスペック--cpu,memoryは良しなに設定ください
  config.vm.provider "virtualbox" do |vb|
    # 表示名、GUIの使用、CPU数、メモリ
    vb.name = "ddd-laravel-doctrine"
    vb.gui = false
    vb.cpus = "4"
    vb.memory = "8192"
  end
